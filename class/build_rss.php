...
// constant
	var $_CACHE_ID_GUEST = HAPPY_LINUX_RSS_CACHE_ID_GUEST;
	var $_CACHE_ID_USER  = HAPPY_LINUX_RSS_CACHE_ID_USER;

	var $_MODULE_ID_DEFUALT = 1;
	var $_HEADER_BUILD = 'Content-Type:text/xml;  charset=utf-8';
	var $_HEADER_VIEW  = 'Content-Type:text/html; charset=utf-8';
	var $_RSS_DOCS     = 'http://backend.userland.com/rss/';
	var $_MAX_ITEMS = 20;
	var $_CACHE_TIME_ONE_HOUR = 3600;	// one hour 
...
// set param
	var $_channel = array();
	var $_items   = array();
	var $_view_title       = 'view cache';
	var $_view_goto_title  = 'goto index';
	var $_view_goto_url    = null;
 	var $_flag_default_timezone = false;
 
 	var $_cache_time_guest = 0;
 	var $_cache_time_user  = 0;
	var $_flag_force_guest = false;
	var $_flag_force_user  = false;

	var $_media_group_default = null;
...
// override
	...
	var $_TITLE_RDF      = 'RDF Feeds';
	var $_TITLE_RSS      = 'RSS Feeds';
	var $_TITLE_ATOM     = 'ATOM Feeds';
	var $_TITLE_OTHER    = 'Feeds';
	var $_TEMPLATE_RDF   = null;
	var $_TEMPLATE_RSS   = null;
	var $_TEMPLATE_ATOM  = null;
	var $_TEMPLATE_OTHER = null;
	var $_MODE_BUILD     = 'other';
...
function clear_all_guest_cache()
{
...
function set_header_build($value)
{
...
function set_template_rdf($value)
{
	$this->_TEMPLATE_RDF = $value;
}

function set_template_rss($value)
{
	$this->_TEMPLATE_RSS = $value;
}

function set_template_atom($value)
{
	$this->_TEMPLATE_ATOM = $value;
}

function set_template_other($value)
{
...
function set_other_template($value)
{
	$this->_TEMPLATE_OTHER = $value;
}

function set_title_rdf($value)
{
	$this->_TITLE_RDF = $value;
}

function set_title_rss($value)
{
	$this->_TITLE_RSS = $value;
}

function set_title_atom($value)
{
	$this->_TITLE_ATOM = $value;
}

function set_title_other($value)
{
...
function set_other_title($value)
{
	$this->_TITLE_OTHER = $value;
}

function set_flag_default_timezone($val)
{
	$this->_flag_default_timezone = (bool)$val;
}

function set_cache_time_guest($value)
{
	$this->_cache_time_guest = intval($value);
}

function set_cache_time_user($value)
{
	$this->_cache_time_user = intval($value);
}

function set_flag_force_guest($value)
{
	$this->_flag_force_guest = (bool)$value;
}

function set_flag_force_user($value)
{
	$this->_flag_force_user = (bool)$value;
}

function set_cache_id_guest($value)
{
	$this->_CACHE_ID_GUEST = $value;
}

function set_cache_id_user($value)
{
	$this->_CACHE_ID_USER = $value;
}

function set_media_group_default($value)
{
...
function &_build_rdf_item_default( &$item )
{
...
{
	$channel_list = array(
		'title', 'link', 'docs', 'generator', 'category', 'copyright',
		'language', 'webmaster', 'lastbuild', 'pubdate', 'description',
		'managingeditor', 'atom_link'
	);

	$image_list = array(
		'url', 'width', 'height', 'title', 'link'
	);

	$item_list = array(
		'link', 'guid', 'title', 'description', 'category', 'pubdate',
		'dc_creator', 
		'content_encoded',
		'geo_lat', 'geo_long',
		'georss_point',
		'media_group', 'media_title', 'media_description',
		'media_text', 'media_keywords', 'media_credit',
		'media_content_url', 'media_content_type', 'media_content_medium',
		'media_content_filesize', 'media_content_height', 'media_content_width', 
		'media_thumbnail_url', 'media_thumbnail_height', 'media_thumbnail_width',
		'media_thumbnail_medium_url', 'media_thumbnail_medium_height', 'media_thumbnail_medium_width',
		'media_thumbnail_large_url', 'media_thumbnail_large_height', 'media_large_thumbnail_width'
	);
...
function &_build_rss_item_default( &$item )
{
...
{
	$channel_list = array(
		'title', 'link_alt', 'link_self', 'id', 'rights', 'updated',
		'generator', 'generator_uri',
		'author_name', 'author_uri', 'author_email'
	);

	$item_list = array(
		'link', 'id', 'title', 'content', 'summary', 'category',
		'updated', 'published', 
		'author_name', 'author_uri', 'author_email'
	);
...
function &_build_atom_entry_default( &$entry )
{
...
function _get_item_pubdate( $item )
{
	$val = null;
...
function _get_item_georss_point( $item )
{
	$val = null;
	...
	elseif ( isset($item['geo_lat'])  && ( $item['geo_lat'] != '' ) &&
	         isset($item['geo_long']) && ( $item['geo_long'] != '' ) )
	{
...
function _get_item_media_group( $item )
{
	$val = null;
...
function _assign_channel( &$tpl, &$list, &$channel, $prefix='channel' )
{
...
function _assign_image( &$tpl, &$list, &$channel )
{
	foreach ( $list as $name )
	{
		$key = 'image_'. $name;
...
function _assign_items( &$tpl, &$list, &$items, $tpl_key='items' )
{
	foreach ($items as $item)
	{
...
function _utf8_item( &$tpl, &$list, &$item  )
{
	$arr = array();
	foreach ( $list as $name )
	{
...
// class date
//---------------------------------------------------------
function _set_default_timezone()
{
...
function _date_rfc822( $time )
{
	return $this->_date->date_rfc822_user( $time );
}

function _date_iso8601( $time )
{
