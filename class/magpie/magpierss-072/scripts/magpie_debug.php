...
if (isset($_GET['url'])) {
    ...
    $url = 'https://magpierss.sf.net/test.rss';
}
...
function test_library_support()
{
    ...
    if (!(function_exists('iconv') and function_exists('mb_convert_encoding'))) {
        echo '<b>Warning:</b>  No support for iconv (--with-iconv) or multi-byte strings (--enable-mbstring).' . "No support character set munging.<br>\n";
    } else {
