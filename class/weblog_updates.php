...
{
	var $_HTTP_METHOD  = 'POST';
	var $_CONTENT_TYPE = 'text/xml';
...
// constructor
//---------------------------------------------------------
function happy_linux_weblog_updates()
{
// class
// Assigning the return value of new by reference is deprecated
...
function send_pings_by_param( $param )
{
	...
	$timeout_connect = isset($param['timeout_connect']) ? intval($param['timeout_connect']) : 0;
	$timeout_read    = isset($param['timeout_read'])    ? intval($param['timeout_read'])    : 0;
...
function set_timeout_connect( $time )
{
	if ( intval($time) > 0 )
	{
...
function set_timeout_read( $time )
{
	if ( intval($time) > 0 )
	{
