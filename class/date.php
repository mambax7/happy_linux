...
{
	var $_time_now;
	var $_tz_orignal = null;
	var $_tz_current = null ;
	var $_offset_orginal = null ;
	var $_offset_current = null ;
	var $_flag_revision = false ;
...
// public
//---------------------------------------------------------
function judge_newday( $time, $days )
{
	if ( ( $days > 0 ) &&
...
function judge_today( $time, $hours )
{
	if ( ( $hours > 0 ) &&
...
{
	return $this->get_timezone_name_from_abbr(
		'', $this->hour_to_sec( $hour ), 0 );
}
...
//---------------------------------------------------------
function date_rfc822_user( $time )
{
...
//---------------------------------------------------------
function date_iso8601_user( $time )
{
...
function date_year_user( $time=null )
{
	if ( empty($time) ) {
		$time = time();
	}
...
function get_xoops_user_timezone_offset()
{
	global $xoopsUser;
...
function format_timestamp( $time, $format, $timeoffset='' )
{
