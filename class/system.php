...
function in_array_language( &$arr )
{
...
function &get_user_groups_anonymous()
{
...
function is_active_module_by_dirname( $dirname )
{
	$act = false;
	$module =& $this->get_module_by_dirname( $dirname );
	if ( is_object($module) )
...
// XC 2.1
function is_active_legacy_module()
{
...
//--------------------------------------------------------
function &get_groupperm_mid_list( $gperm_name='module_read', $gperm_modid=1, $gperm_groupid=null )
{
...
function check_groupperm_right( $gperm_name, $gperm_itemid, $gperm_groupid, $gperm_modid=1 )
{
	$groupperm_handler =& xoops_gethandler( 'groupperm' );
...
//--------------------------------------------------------
function check_config_search_enable_search()
{
...
function get_config_search_keyword_min()
{
	$config_handler =& xoops_gethandler('config');
	$xoopsConfigSearch =& $config_handler->getConfigsByCat( XOOPS_CONF_SEARCH );
...
// XoopsLists
//--------------------------------------------------------
function &get_img_list_as_array( $dir )
{
...
function assign_template( $varname, $var=null )
{
	global $xoopsTpl;
	if ( is_object($xoopsTpl) )
	{
...
function add_template( $varname, $var, $glue='' )
{
