<?php
// $Id: myblockform.php,v 1.1 2007/05/15 04:56:01 ohwada Exp $

//=========================================================
// Happy Linux Framework Module
// This file is same as GIJOE's myblocksadmin
//=========================================================
...
if ($block['is_custom']) {
    ...
    $textarea->setDescription(
        '<span style="font-size:x-small;font-weight:bold;">' . _AM_USEFULTAGS . '</span><br><span style="font-size:x-small;font-weight:normal;">' . sprintf(
            _AM_BLOCKTAG1,
            '{X_SITEURL}',
            XOOPS_URL . '/'
        ) . '</span>'
    ...
    if ('' != $block['template']) {
        ...
        if (count($btemplate) > 0) {
            $form->addElement(
                new \XoopsFormLabel(
                    _AM_CONTENT, '<a href="' . XOOPS_URL . '/modules/system/admin.php?fct=tplsets&op=edittpl&id=' . $btemplate[0]->getVar('tpl_id') . '">' . _AM_EDITTPL . '</a>'
                )
            );
        } else {
            ...
            if (count($btemplate2) > 0) {
                $form->addElement(
                    new \XoopsFormLabel(
                        _AM_CONTENT, '<a href="' . XOOPS_URL . '/modules/system/admin.php?fct=tplsets&op=edittpl&id=' . $btemplate2[0]->getVar('tpl_id') . '" target="_blank">' . _AM_EDITTPL . '</a>'
                    )
                );
            }
        }
